...
//            mPaint.setPathEffect(mDashPathEffect);
        ...
        if (mEffCtrlPtId == e_moving) {
            ...
            for (int i = 0; i < CTRL_PTS_CNT; ++i) {
                ...
                if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                    ...
                    PointF pt = mAnnotView.getDrawingView().getPolyShapeCtrlPts()[i];
                    if (pt != null) {
                        pt.x = mCtrlPts[i].x - xOffset;
                        pt.y = mCtrlPts[i].y - yOffset;
                    }
                }
            }
            updateAnnotView();
            ...
            if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                ...
                PointF pt = mAnnotView.getDrawingView().getPolyShapeCtrlPts()[mEffCtrlPtId];
                if (pt != null) {
                    mAnnotView.getDrawingView().getPolyShapeCtrlPts()[mEffCtrlPtId].x = mCtrlPts[mEffCtrlPtId].x - xOffset;
                    mAnnotView.getDrawingView().getPolyShapeCtrlPts()[mEffCtrlPtId].y = mCtrlPts[mEffCtrlPtId].y - yOffset;
                }
            }
    ...
    protected void updateAnnotView() {
        if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
            ...
            mAnnotView.getDrawingView().setAnnotRect(new android.graphics.Rect((int) x1, (int) y1, (int) x2, (int) y2));
            int xOffset = mPdfViewCtrl.getScrollX();
            int yOffset = mPdfViewCtrl.getScrollY();
