...
public class Pan extends Tool {
    ...
    protected QuickMenu createQuickMenu() {
        ...
        quickMenu.addMenuEntries(((QuickMenuBuilder) quickMenu.getMenu()).getMenuItems(), QM_MAX_ROW_SIZE);
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        ...
        boolean isIRT = false;
        try {
            isIRT = AnnotUtils.hasInReplyTo(mAnnot);
        } catch (Exception ex) {
            AnalyticsHandlerAdapter.getInstance().sendException(ex);
        }

        if (mAnnot != null && !isIRT) {
            ...
            try {
                ...
                ArrayList<Annot> annotsInGroup = AnnotUtils.getAnnotationsInGroup(mPdfViewCtrl, mAnnot, mAnnotPageNum);
                if (annotsInGroup != null && annotsInGroup.size() > 1) {
                    // this is a group of annotations
                    mNextToolMode = ToolMode.ANNOT_EDIT_RECT_GROUP;
                    mGroupAnnots = annotsInGroup;
                }
            } catch (Exception ex) {
                AnalyticsHandlerAdapter.getInstance().sendException(ex);
            }
    ...
    public boolean onQuickMenuClicked(QuickMenuItem menuItem) {
        ...
        if (menuItem.getItemId() == R.id.qm_line) {
            ...
            ToolManager.Tool tool = toolManager.createTool(ToolMode.RECT_REDACTION, this);
            toolManager.setTool(tool);
        } else {
