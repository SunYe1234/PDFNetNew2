        ...
        try {
            ...
            if (pageRect != null) {
                for (Annot annot : annotsInPage) {
                    ...
                    if (annotRect.intersectRect(pageRect, annotRect) && hasPermission(annot, ANNOT_PERMISSION_SELECTION) && isAnnotSupportEdit(annot)) {
    ...
    protected void flattenAnnot() {
        ...
        try {
            ...
            for (Map.Entry<Annot, Integer> entry : mSelectedAnnotsMap.entrySet()) {
                ...
                int annotPageNum = entry.getValue();
                AnnotUtils.flattenAnnot(mPdfViewCtrl, annot, annotPageNum);
            }
