...
public class AnnotEdit extends Tool implements DialogAnnotNoteListener,
        ...
        try {
            ...
            if (mAnnot.isMarkup() && type == Annot.e_Stamp) {
                ...
                if (sigObj != null) {
                    ...
                    mRotateStartDegree = AnnotUtils.getStampDegree(mAnnot);
                }
        ...
        if (mRotateHandle != null) {
            mRotateHandle.setListener(new RotateHandleView.RotateHandleViewListener() {
                ...
                public void onDown(float rawX, float rawY) {
                    closeQuickMenu();
                    mRotateDown.set(rawX, rawY);
                }

                @Override
                public void onMove(float rawX, float rawY) {
                    mRotateMove.set(rawX, rawY);
                    if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                        RotateInfo info = mAnnotView.getDrawingView().handleRotation(mRotateDown, mRotateMove, false);
                        ...
                        if (totalDegree > 0) {
                            if (Math.abs(totalDegree - 45) < mRotateThreshold) {
                                mSnapDegree = 45;
                                canSnap = true;
                            } else if (Math.abs(totalDegree - 90) < mRotateThreshold) {
                                mSnapDegree = 90;
                                canSnap = true;
                            } else if (Math.abs(totalDegree - 135) < mRotateThreshold) {
                                mSnapDegree = 135;
                                canSnap = true;
                            } else if (Math.abs(totalDegree - 180) < mRotateThreshold) {
                                mSnapDegree = 180;
                                canSnap = true;
                            } else if (Math.abs(totalDegree - 225) < mRotateThreshold) {
                                mSnapDegree = 225;
                                canSnap = true;
                            ...
                            if (Math.abs(Math.abs(totalDegree) - 45) < mRotateThreshold) {
                                mSnapDegree = -45;
                                canSnap = true;
                        ...
                        if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                            mAnnotView.getDrawingView().snapToDegree(mSnapDegree, mRotateStartDegree);
                        }
                ...
                public void onUp(float rawX, float rawY, float x, float y) {
                    mRotateMove.set(rawX, rawY);
                    if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
                        RotateInfo info = mAnnotView.getDrawingView().handleRotation(mRotateDown, mRotateMove, true);
                        mRotateDegree = info.getDegree();
                    }
                    ...
                    if (mRotateHandle != null) {
                        int l = (int) (rawX - x + 0.5);
                        int t = (int) (rawY - y + 0.5);
    ...
    public void onDraw(Canvas canvas, Matrix tfm) {
        ...
        if (!mAnnotIsStamper) {
            // draw stamp in drawing view due to rotation
    ...
    protected void changeAnnotAppearance() {
        ...
        try {
            ...
            mAnnotStyleDialog = styleDialogBuilder.setAnnotStyle(mAnnotStyle).setAnchor(annotRect).build();
    ...
    protected void rotateStampAnnot(int degree, boolean snap) {
        ...
        try {
            ...
            int rotation = AnnotUtils.getStampDegree(mAnnot);
    ...
    protected void updateCtrlPts(boolean translate,
        ...
        updateRotateView((int) (left + 0.5),
                (int) (top + 0.5),
                (int) (right + 0.5),
                (int) (bottom + 0.5));
    }

    protected void updateAnnotView(int left, int top, int right, int bottom) {
        if (mAnnotView != null && mAnnotView.getDrawingView() != null) {
            int xOffset = mPdfViewCtrl.getScrollX();
            int yOffset = mPdfViewCtrl.getScrollY();
            mAnnotView.getDrawingView().setAnnotRect(
    ...
    protected void updateRotateView(float min_x, float min_y, float max_x, float max_y) {
        if (mRotateHandle != null) {
            int size = (int) Utils.convDp2Pix(mPdfViewCtrl.getContext(), DrawingUtils.sRotateHandleSize);
    ...
    protected void updateAnnotViewCtrlPt() {
        if (mAnnotView != null) {
            ...
            ctrlPts[e_ul] = new PointF(mCtrlPts[e_ul].x - xOffset, mCtrlPts[e_ul].y - yOffset);
            ctrlPts[e_lr] = new PointF(mCtrlPts[e_lr].x - xOffset, mCtrlPts[e_lr].y - yOffset);
            ctrlPts[e_lm] = new PointF(mCtrlPts[e_lm].x - xOffset, mCtrlPts[e_lm].y - yOffset);
            ctrlPts[e_ml] = new PointF(mCtrlPts[e_ml].x - xOffset, mCtrlPts[e_ml].y - yOffset);
    ...
    private void editTextSize(float textSize) {
        ...
        try {
            ...
            resizeFreeText(freeText, freeText.getContentRect(), isRightToLeft);
    ...
    private void editRuler(RulerItem rulerItem) {
        ...
        try {
            ...
            if (AnnotUtils.isRuler(mAnnot) ||
                    AnnotUtils.isPerimeterMeasure(mAnnot) ||
                    AnnotUtils.isAreaMeasure(mAnnot)) {
                raiseAnnotationPreModifyEvent(mAnnot, mAnnotPageNum);
                ...
                } else if (AnnotUtils.isPerimeterMeasure(mAnnot)) {
                    PolyLine polyLine = new PolyLine(mAnnot);
                ...
                } else if (AnnotUtils.isAreaMeasure(mAnnot)) {
                    Polygon polygon = new Polygon(mAnnot);
    ...
    private void editRedactionOverlayText(String overlayText) {
        ...
        try {
            ...
            Redaction redaction = new Redaction(mAnnot);
    ...
    private void updateFreeText(String contents) {
        ...
        try {
            ...
            if (!contents.equals("")) {
                ...
                ToolManager toolManager = (ToolManager) mPdfViewCtrl.getToolManager();
                if (toolManager.isDeleteEmptyFreeText() || !oldContents.equals(contents)) {
        ...
        try {
            ...
            if (sz > 0) {
                for (int i = 0; i < sz; ++i) {
                    ...
                    if (i == 0) {
                        minX = Math.min(quadRect.getX1(), quadRect.getX2());
                        maxX = Math.max(quadRect.getX1(), quadRect.getX2());
                        minY = Math.min(quadRect.getY1(), quadRect.getY2());
                        maxY = Math.max(quadRect.getY1(), quadRect.getY2());
                    } else {
                        minX = Math.min(minX, Math.min(quadRect.getX1(), quadRect.getX2()));
                        maxX = Math.max(maxX, Math.max(quadRect.getX1(), quadRect.getX2()));
                        minY = Math.min(minY, Math.min(quadRect.getY1(), quadRect.getY2()));
                        maxY = Math.max(maxY, Math.max(quadRect.getY1(), quadRect.getY2()));
                    }

                    rarr.add(new Redactor.Redaction(mAnnotPageNum, quadRect, false, overlayText));
                }
            }
            ...
            app.border = false;
            Redactor.redact(mPdfViewCtrl.getDoc(), rarr.toArray(new Redactor.Redaction[rarr.size()]), app, false, false);
