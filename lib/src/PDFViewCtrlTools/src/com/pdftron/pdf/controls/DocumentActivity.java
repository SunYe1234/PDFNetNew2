...
public class DocumentActivity extends AppCompatActivity implements
    ...
    public static void openDocuments(Context packageContext, ArrayList<Uri> fileUris, @Nullable ViewerConfig config) {
        Intent intent = new Intent(packageContext, DocumentActivity.class);
        if (null != fileUris) {
            intent.putParcelableArrayListExtra(EXTRA_FILE_URI_LIST, fileUris);
        }
        intent.putExtra(EXTRA_CONFIG, config);
        packageContext.startActivity(intent);
    }
    ...
    /**
     * Use {@link ViewerBuilder#createBundle(Context)} instead.
     */
    @Deprecated
    ...
    protected void onDocumentSelected(Uri fileUri, String password) {
        ViewerBuilder builder = ViewerBuilder.withUri(fileUri, password);
    ...
    protected void startTabHostFragment(@NonNull ViewerBuilder builder) {
        ...
        builder.usingNavIcon(R.drawable.ic_arrow_back_white_24dp)
    ...
    /**
     * @see #startTabHostFragment(ViewerBuilder)
     */
    @Deprecated
    protected void startTabHostFragment(Bundle args) {
        ...
        if (mToolbarMenuResArray != null) {
            args.putIntArray(PdfViewCtrlTabHostFragment.BUNDLE_TAB_HOST_TOOLBAR_MENU, mToolbarMenuResArray);
        }
